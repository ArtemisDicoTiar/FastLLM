...
@torch.no_grad()
def speculative_decoding(input_ids:Tensor, drafter:Module, target:Module, min_len:int=20, gamma:int = 4,
                         temperature:float = 1, top_k:int = 0, top_p:float = 0, end_token_id:int=1, debug = False) -> Tuple[Tensor, float]:
    """
    ...
    :param min_len: minimum length of the output sequence.
    ...
    :param end_token_id: end token id to stop generating.
    ...
    seq = input_ids
    seq_len = seq.shape[1]
    maximum_length = seq_len + min_len
    total_accept = 0
    number_of_trials = 0
    ...
    while seq_len < maximum_length:
        ...
        p_p = norm_logits(Mp.logits[..., -1, :].squeeze(0), temperature, top_k, top_p)[:drafts_probs[0].shape[0]]
        if n < gamma:
            p_p = max_fn(p[n] - draft_logits[n])
